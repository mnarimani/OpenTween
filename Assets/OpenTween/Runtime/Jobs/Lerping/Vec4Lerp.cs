...
namespace OpenTween.Jobs
{
    ...
    internal struct Vec4Lerp : IJobParallelFor
    {
        ...
        public unsafe void Execute(int i)
        {
            ...
            ref TweenInternal<Vector4> t = ref UnsafeUtility.ArrayElementAsRef<TweenInternal<Vector4>>(Array.GetUnsafePtr(), index);

            if (Hint.Unlikely(!t.IsUpdatedInLastFrame))
                return;

            ref TweenOptions<Vector4> options = ref UnsafeUtility.ArrayElementAsRef<TweenOptions<Vector4>>(Options.GetUnsafePtr(), index);

            Vector4 end = options.IsRelative ? options.Start + options.End : options.End;
